...
def get_images_with_progress(ws, workflow, client_id, base_url, api_key):
    """
    Generator that yields progress updates and final images from ComfyUI.
    
    Yields:
        dict: Progress events with format {"type": "progress", "step": int, "total": int, "percentage": float}
              or final result {"type": "complete", "data": list[dict]}
    """
    prompt_id = queue_prompt(workflow, client_id, base_url, api_key)["prompt_id"]
    output_images = []
    ...
    while True:
        ...
        if isinstance(out, str):
            ...
            if msg_type == "progress":
                # ComfyUI sends: {"type": "progress", "data": {"value": 15, "max": 50}}
    ...
    yield {"type": "complete", "data": output_images}
...
async def comfyui_create_image_with_progress(
    model: str, payload: ComfyUICreateImageForm, client_id, base_url, api_key, on_progress=None
):
    """
    Create an image using ComfyUI with progress callbacks.
    
    Args:
        model: The model name to use
        payload: The image creation payload
        client_id: WebSocket client ID
        base_url: ComfyUI base URL
        api_key: ComfyUI API key
        on_progress: Optional async callback function that receives progress events
                     with format {"step": int, "total": int, "percentage": float}
    
    Returns:
        dict with "data" containing list of generated image URLs
    """
    ws_url = base_url.replace("http://", "ws://").replace("https://", "wss://")
    workflow = json.loads(payload.workflow.workflow)
    ...
    try:
        log.info("Sending workflow to WebSocket server with progress tracking.")
        log.info(f"Workflow: {workflow}")
        
        # Use get_images_with_progress generator
        images = None
        for event in get_images_with_progress(ws, workflow, client_id, base_url, api_key):
