...
from open_webui.utils.images.comfyui import (
    ...
    comfyui_create_image_with_progress,
    comfyui_edit_image,
    get_images_with_progress,
)
...
async def generate_images_stream(
    ...
    """
    SSE streaming endpoint for image generation with progress updates.
    Only works with ComfyUI engine - other engines return immediately.
    
    SSE Events:
        - progress: {"type": "progress", "step": 15, "total": 50, "percentage": 30.0}
        - complete: {"type": "complete", "images": [{"url": "..."}]}
    ...
    # Only ComfyUI supports streaming progress
    if request.app.state.config.IMAGE_GENERATION_ENGINE != "comfyui":
        # For non-ComfyUI engines, fall back to regular generation
        images = await image_generations(request, form_data, user=user)
        
        async def non_streaming_response():
            yield f"data: {json.dumps({'type': 'complete', 'images': images})}\n\n"
    ...
    async def comfyui_stream_generator():
        try:
            # Parse size
            size = "512x512"
            if (
            ...
            ws.connect(f"{ws_url}/ws?clientId={user.id}", header=headers)

            try:
                # Use the progress generator in a thread
                def stream_from_ws():
                    return list(get_images_with_progress(
                        ws, workflow, user.id, base_url, api_key
                    ))

                # Run synchronous generator in thread and yield results
                for event in await asyncio.to_thread(stream_from_ws):
            ...
            log.exception(f"Error in ComfyUI streaming: {e}")
            yield f"data: {json.dumps({'type': 'error', 'message': str(e)})}\n\n"

    return StreamingResponse(
        comfyui_stream_generator(),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "Connection": "keep-alive",
...
async def image_generations(
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            # Use progress-enabled function if event_emitter is provided
            if event_emitter:
                async def on_progress(progress):
                    await event_emitter({
                        ...
                        "data": {
                            ...
                            "description": f"Generating image ({progress['percentage']:.0f}%)",
                            "progress": progress,
                            "done": False,
                        },
                    })
                
                res = await comfyui_create_image_with_progress(
                    model,
                    form_data,
                    user.id,
